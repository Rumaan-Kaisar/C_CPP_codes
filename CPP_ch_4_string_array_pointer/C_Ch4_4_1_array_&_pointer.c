...
/* Example 0: What is generated when you use an array nanle without an index? */
An array name with no index generates a pointer to the start of the array
...
/* Example 12: Write a function called avg() that averages a list of floating-point values. 
                ...
                The first is a pointer to the array containing the numbers; 
                the second is an integer value, which specifies the size of the array. 
...
void avg(double *d, int num){
    ...
    temp = num-1;
    for(sum=0; temp>=0; temp--) sum = sum + d[temp];
