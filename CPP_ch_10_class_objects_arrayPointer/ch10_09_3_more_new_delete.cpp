...
/* Example 10: Can dynamically allocated arrays be initialized? 
...
In C++, when you dynamically allocate an array using "new", 
    you can't directly initialize it with a "list of values" because the memory is allocated at "runtime", 
    and there's no syntax for providing an initializer list like with a "static array".
    ...
    When you dynamically allocate an array in C++ using "new", 
        ...
        This is because:
            ...
            Compiler Limitations: 
                The compiler doesn't have enough information at compile time to 
                    know the "specific values" you want to initialize the array with, 
                    especially if those values are determined by user input or calculations.

            Efficiency: 
                Initializing every element to a default value (like 0) during allocation 
                    might introduce unnecessary overhead, especially for large arrays.


Therefore, to initialize a dynamically allocated array, 
    you must manually assign values to each element after the allocation using a "loop" or other methods. 
    This gives you flexibility in setting the initial values based on your program's logic.
...
Static Arrays: 
    When you create a static array (e.g., int arr[5] = {1, 2, 3, 4, 5};), 
    the initializer list is used at "compile-ti"me to allocate and initialize the array.

Dynamic Arrays: 
    With dynamic arrays, the memory is allocated at "runtime", 
    so there's no mechanism in the language to take an initializer list and 
    apply it to the dynamically allocated memory.
...
How to Initialize Dynamically Allocated Arrays:
    ...
    2. Use std::vector: 
        If you need the convenience of initialization and dynamic sizing, 
        use std::vector, which allows initializing with a list of values.

                std::vector<int> arr = {1, 2, 3, 4, 5};


    i.e. you need to manually initialize each element 
        or use higher-level containers like std::vector.
