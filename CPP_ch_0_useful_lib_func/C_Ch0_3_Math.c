...
/*  ANSI C defines several mathematics functions that take 'double' arguments and 'return double values'. Function categories:
    ...
    All the math functions require that the header "<math.h>" be included in any program that uses them.  
        This header defines a 'macro' called "huge_val" for overflowing double causing RANGE ERROR. 
        A DOMAIN ERROR occurs if the input value is not in the domain. 

    ANGLES are specified in RADIANS. 
*/

#include <math.h>    // must be  included before use	Example
...
/* 
    pow() returns 'base^exp'.
        A domain error may occur if base=0 and exp≤0.  
        A domain error will occur if base<0 and exp is not an integer. 
