...
#include <stdlib.h>
void abort(void);	abort() causes immediate termination of a program. Whether it closes any open files is defined by the implementation, but generally it won’t.	for(;;) if(getche()=='A') abort();
