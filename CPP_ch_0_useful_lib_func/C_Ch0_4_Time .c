...
// #include <time.h>    must be  included before use	Example

#include <time.h>


char *asctime(const struct tm *ptr);
/* asctime() returns a 'pointer to a string' that contains the TIME and DATE 
    stored in the structure pointed to by 'ptr' after it has been converted into the following form:
    ...
    struct pointer passed to asctime() is generally obtained from either localtime() or gmtime(). 
        The buffer used by asctime() to hold the formatted output string is a statically allocated character array and 
        is overwritten each time the function is called. 
        
        To save the contents of the string, copy it elsewhere.	
    Eg:     
        struct tm *ptr; 
        time_t It;
...
/* clock() returns the 'number of system clock cycles' that have occurred since the PROGRAM began EXECUTION. 
    ...
    int main(void){
        ...
        for(i=0; i<100; i++) printf("%u", clock()); 
